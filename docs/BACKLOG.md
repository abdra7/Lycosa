...
- **#28** RAG `.docx` silent corruption — SHIPPED 2026-07-12 (ADR-025):
  python-docx loader (paragraphs + table rows) and a binary-content guard on
  the plain-text fallback (ZIP/OLE2 magic, NULs, replacement-char ratio →
  `ExtractionError` instead of embedding junk).
- **#29** RAG scanned/image-only PDFs — SHIPPED 2026-07-12 (ADR-026): no-text-
  layer PDFs now fail with an actionable "looks like a scan" message; OCR
  opt-in via the backend `[ocr]` extra (pytesseract + pillow + tesseract).
