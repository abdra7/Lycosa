...
class _NodesGraphViewState extends State<NodesGraphView>
  ...
  void _seed() {
    ...
    for (var i = 0; i < count; i++) {
      ...
      final start = existing[n.id]?.position ??
          center + Offset.fromDirection(angle, 140);
  ...
  void _onTick(Duration elapsed) {
    ...
    for (var i = 0; i < list.length; i++) {
      for (var j = i + 1; j < list.length; j++) {
        ...
        if (dist < 1) {
          delta = Offset(math.Random(i * 31 + j).nextDouble() - 0.5,
    ...
    for (final b in list) {
      ...
      b.position = Offset(
        b.position.dx.clamp(margin, math.max(margin, _canvasSize.width - margin)),
        b.position.dy.clamp(margin, math.max(margin, _canvasSize.height - margin)),
      );
    }
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    if (widget.nodes.isEmpty) {
      return const Center(
          child: Text('No nodes yet. Add one and run lycosa-agent on the machine.'));
    }
    return Column(
            ...
            return GestureDetector(
              ...
              child: CustomPaint(
                ...
                painter: _GraphPainter(
                  ...
                  mutedTextColor: Theme.of(context).colorScheme.onSurfaceVariant,
...
class _GraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _drawLabel(canvas, 'Controller', hub.position + Offset(0, hubRadius + 14),
        mutedTextColor, bold: false);
    ...
    for (final b in bodies.values) {
      ...
      _drawGlyph(canvas, b.position,
          _roleGlyphs[node.role ?? node.recommendedRole] ?? '?', Colors.white);
      _drawLabel(canvas, node.name, b.position + Offset(0, nodeRadius + 14),
          textColor, bold: true);
    }
  }

  // statusColor() needs a BuildContext for theme lookups elsewhere, but the
  // painter only needs the three fixed hues — duplicated here deliberately
  // narrow rather than threading a context into paint().
  Color statusColorFor(String status) => switch (status) {
        'online' => Colors.green,
        'offline' => const Color(0xFFD03B3B),
        _ => const Color(0xFFFAB219),
      };
  ...
  Widget build(BuildContext context) {
    ...
    return Wrap(
      ...
      children: [
        ...
        _statusDot(context, Colors.green, 'online'),
        _statusDot(context, const Color(0xFFFAB219), 'registered'),
        _statusDot(context, const Color(0xFFD03B3B), 'offline'),
      ],
    );
  }
